...
class Player
{
	...
	char emotion[100];
public:
	Player(int pos = 20, const char* face = "(^,.^)") : position(pos) { strcpy(this->emotion, face); }
	void move(int num)
	{
		this->position += num;
	}
...
class Enemy
{
	int position;
	char emotion[100];
	char emotion_attacked[100];
	int sec;
	bool attacked;
public:
	Enemy(int pos = 50, char* face = "(天,.天)", char* face_attacked = "(仄,.仄)") :position(pos), sec(-1), attacked(false) { strcpy(this->emotion, face);	strcpy(this->emotion_attacked, face_attacked); }
	void move(int num)
	{
		this->position += num;
	}
...
class Bullet
{
	...
	char shape;
public:
	Bullet(int player_pos = -1) : position(player_pos), shape('-') {}
